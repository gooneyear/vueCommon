<template>
  <div id="wd-Evalreport">
    ...
    <div v-else>
      <div v-if="showList==0">
        <header>
          <span style="margin-top: 20px; margin-bottom: 20px">当前没有言语测评记录</span>
          <br>
      ...
      <header>
        ...
        <div v-if="showList == 1">
          <span>
            <!--
            <a href="javascript:void(0);" class="wd-reporttag" v-for="(ret,index) in canreportlists">
              <span @click="getRecordDetails(ret.id)">{{ret.modifyTime | time}}</span>
              <span @click="getRecordDetails(ret.id)" :title="ret.scalename">{{ret.scalename}}</span>
            </a>
            -->
            <a href="javascript:void(0);" class="wd-reporttag wd-normaltag" v-for="(ret,index) in canreportlists" @click="getactive(index)" :class="{'active':index==0}">
              <span @click="ret.trigger()">{{ret.assessTime | time}}</span>
              <span @click="ret.trigger()">{{ret.title}}</span>
            </a>
          </span>
      ...
      <div v-if="showList==1 && showWAB == 0 && show_1_0 == 1">
        <header>
          <h2>认知测评</h2>
          <h4>{{ curr_1_0_record.comment.SCALENAME }}</h4>
        </header>
        ...
        <div id="report-content">
          <div id="content-basic">
            ...
            <table>
              ...
              <tr>
                <th>姓　　名：</th>
                <td>	{{curr_1_0_record.userdatas.user_realname}}</td>
                <th>联系方式：</th>
                <td>{{curr_1_0_record.userdatas.user_tel}}</td>
              </tr>
              <tr>
                <th>文　　化：</th>
                <td v-if="curr_1_0_record.userdatas.user_education==1">文盲</td>
                <td v-else-if="curr_1_0_record.userdatas.user_education==2">小学</td>
                <td v-else-if="curr_1_0_record.userdatas.user_education==3">初中</td>
                <td v-else-if="curr_1_0_record.userdatas.user_education==4">高中</td>
                <td v-else-if="curr_1_0_record.userdatas.user_education==5">专科</td>
                <td v-else-if="curr_1_0_record.userdatas.user_education==6">本科及以上</td>
                <td v-else-if="curr_1_0_record.userdatas.user_education==''">其他</td>
                <td v-else>其他</td>
                <!--<td v-else-if="curr_1_0_record.userdatas.user_education==''">未知</td>-->
                <th>性　　别：</th>
                <td v-if="curr_1_0_record.userdatas.user_gender == 1">女</td>
                <td v-else-if="curr_1_0_record.userdatas.user_gender == 2">男</td>
                <td v-else>其他</td>
              </tr>
              <tr>
                <th>利　　手：</th>
                <td v-if="curr_1_0_record.userdatas.user_hands==1">左利手</td>
                <td v-else-if="curr_1_0_record.userdatas.user_hands==2">右利手</td>
                <td v-else-if="curr_1_0_record.userdatas.user_hands==3">双利手</td>
                <td v-else>右利手</td>
                <th>生　　日：</th>
                <td>{{curr_1_0_record.userdatas.user_birthday}}</td>
              </tr>
              <!--<tr>-->
                <!--<th>既往病史：</th>-->
          ...
          <div id="content-detail">
            ...
            <table>
              <tr>
                <th>项目</th>
                <template v-if="curr_1_0_record.comment.ID=='70'">
              ...
              <tr v-for="item in curr_1_0_record.scoredatas">
                <template v-if="curr_1_0_record.comment.ID=='70'">
            ...
            <div id="score-declare">
              <h5>得分说明</h5>
              <p v-html="curr_1_0_record.comment.RESULTEXPLAIN"></p>
        ...
        <div id="report-footer">
          <button @click="print10(printaid,printuid)" style="">去打印</button>
          <button @click="gotoScale1()" style="cursor:pointer;">确定</button>
        </div>
        </div>

      <div v-if="showList==1 && showWAB == 1 && show_1_0 == 0">
      ...
      <article class="total print_area">
        ...
        <section>
          ...
          <aside class="wd-echart">
              <div id="main" style="width: 100%;height:300px;">123</div>
          </aside>
        ...
        <section>
          <h4>小项报告</h4>
          <aside class="wd-echart">
              <div id="main2" style="width:100%;height:700px;">123</div>
          </aside>
        ...
        <section class="wd-suggest">
          ...
          <textarea name="" rows="10" placeholder="请输入建议..." maxlength="500" class="suggesttext">{{ reportdatas.advice }}</textarea>
        </section>
      ...
      <footer>
        <aside>
          <a href="javascript:void(0);" class="ensure" @click="gotoScale()">确定</a>
          <a href="javascript:void(0);" class="print" @click="printWab(reportid,printuid)" style="">去打印</a>
        </aside>
      ...
      <input type="hidden" class="printuid" :value="printuid"><!--1.0-->
      <input type="hidden" class="printaid" :value="printaid"><!--1.0-->
      <input type="hidden" class="printtype" :value="showWAB"><!--2.0-->
      <input type="hidden" class="printreportid" :value="reportid"><!--2.0-->
      </div>
  </div>
</template>
<script>
...
//        reportLists:reportdas,
    ...
    methods: {
      ...
      gotoScale:function(){
        var self = this;
//        var note = $(".remarktext").val();
        var advice = $(".suggesttext").val();
        if(advice.length>0 && advice.length<600){
            self.$http.post('/api/Evaluation/saveReport', {
              id: self.reportid,
              note: "",
              advice: advice,
            }).then(function (res) {
        ...
        }else if(advice.length<=0 || advice.length>600){
            $('.showred1').css('display','inline-block');
        }
...
//        this.$router.push({ path: '/doc/evaluation/'});
      ...
      gotoScale1:function(){
        var self = this;
        self.$router.push({ path: '/doc/evaluation/'});

      },
      gotoyy:function () {
        this.$router.push({ path: '/doc/cases/evalRecord/'+this.$route.params.id, });
      },
      gotorz:function () {
        this.$router.push({ path: '/doc/cases/recordDetail/'+this.$route.params.id});
      },
              ...
              self.assessRecords.concat(list.map((x)=>
              {
                return {
                  ...
                  trigger: ((val)=>
                  {
                    ...
                    const aid = val.ID;
                    self.printuid=val.PATIENTINFOID;
                    self.printaid=val.ID;
...
//            bottom: '3%',
      ...
      print:function(){
//        var uid = $(".printreportid").val();
        var type = $(".printtype").val();
        var reportid = $(".printreportid").val();
        var aid = $(".printaid").val() ? $(".printaid").val() : 0;
        var advice = $(".suggesttext").val();
//          var docc = document.getElementById("evalRecordContent");
//        window.document.body.innerHTML=docc.innerHTML;
//        if(advice.length>0){
//          this.$router.push({ path: '/print/'+this.$route.params.id +"/"+type+"/"+reportid+"/"+aid});
          window.open('#/print/'+this.$route.params.id +"/"+type+"/"+reportid+"/"+aid);
//        }
//        this.Printpart("print_area");

      },
      print10(printaid,printuid){
        window.open('#/print10/'+printaid+'/'+printuid);
      },
      printWab(printreportid,printuid){
        window.open('#/printwab/'+printreportid+'/'+printuid);
      },
...
//          console.log(1111111111111)
        var previewSwiper = previewSwiper= new Swiper('.preview .swiper-container', {
        ...
        $('.preview .arrow-right').on('click', function(e) {
          e.preventDefault();
//          console.log(previewSwiper.activeIndex,previewSwiper.slides)
...
<style lang="stylus" scoped>
  ...
  a.print
    margin-left 30px
    background-color rgb(27,190,159)
