export default {
  ...
  digits: (field, [length]) => ` ${field}必须为${length}位数字`,
  ...
  in: (field,[target1,target2]) => ` ${field} 必须是 '${target1}' 、 '${target2}' 中的一个值.`,
  ...
  regex: (field) => ` 格式有误，请重填.`,
  required: (field) => `必填项，请填写${field}.`,
  size: (field, [size]) => ` ${field} 必须小于 ${size} KB.`,
  url: (field) => ` ${field}不是有效的url.`,
  max_value: (field, [value]) => ` ${field}最大值是${value}.`,
  mobile: (field)=>``
}
